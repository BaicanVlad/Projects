...
Operation* createOperation(Medication* m, char* operationType)
{
	...
	o->medication = copyMedication(m);

	if (operationType != NULL)
	{
		o->operationType = (char*)malloc((strlen(operationType) + 1));
...
void destroyOperation(Operation* o)
{
	...
	destroyMedication(o->medication);
...
void testsStack()
{
	...
	Medication* m1 = createMedication("MED1",0.76, 100, 79.99);
	Medication* m2 = createMedication("MED2", 0.76, 100, 79.99);
	Operation* o1 = createOperation(m1, "add");
	Operation* o2 = createOperation(m2, "add");
	Operation* o3 = createOperation(m1, "remove");

	// the medications may be destroyed, as the operations contain copies of these planets
